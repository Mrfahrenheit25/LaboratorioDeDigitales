...
int SegDisplCount=0, count = 1, DELAY;
...
{
int SWITCHES, BTN;
SWITCHES = 0x10000 & M_PSP_READ_REGISTER_32(GPIO_SWs);
DELAY = M_PSP_READ_REGISTER_32(RPTC_LRC);
BTN = M_PSP_READ_REGISTER_32(GPIO_BTNs);

if (SWITCHES == 0x10000) count = 0;
if (BTN == 1 && DELAY > 50000000)
{
DELAY = DELAY - 50000000;
}
else if (BTN == 1 && DELAY < 50000000)
{
DELAY = DELAY + 50000000;
}
...
void PTC_ISR(void){
  ...
  // reset = 0b000100001 & 0b111011111;                   // Se debe cambiar el valor del PTC_CTRL para definir la configuración deseada
                                                        // Se hace una máscara para cambiar únicamente el bit 5 y limpiar la interrupc
  M_PSP_WRITE_REGISTER_32(RPTC_CTRL, 0b000100001);           /* PTC_CTRL */
...
{
   M_PSP_WRITE_REGISTER_32(RPTC_CTRL, 0b000100001);   //         /* RPTC_CTRL initialization */

   M_PSP_WRITE_REGISTER_32(RPTC_HRC, 80000000);       /* HRC */
   M_PSP_WRITE_REGISTER_32(RPTC_LRC, 80000000); 
